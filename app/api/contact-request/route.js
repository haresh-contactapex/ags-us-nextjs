...
import { validateContactRequest } from "@/lib/validateContactRequest";
...
export async function POST(request) {
  ...
  try {
    ...
    console.error("contact-request: recaptcha verification error", error);
    return NextResponse.json(
  ...
  try {
    ...
    await sql`
      INSERT INTO contact_requests
        (first_name, last_name, email, phone, service, subject, message, ip_address, user_agent)
      VALUES
        (${data.first_name}, ${data.last_name}, ${data.email}, ${data.phone}, ${data.service}, ${data.subject}, ${data.message}, ${remoteIp}, ${request.headers.get("user-agent")})
    `;
  } catch (error) {
    console.error("contact-request: failed to save to database", error);
    return NextResponse.json(
      { message: "Something went wrong. Please try again later." },
      { status: 500 },
    );
  ...
  emailTasks.push(
    sendMail({
      ...
      html: `
        ...
        <p>Thanks for reaching out to Apex Global Solutions. We've received your message and will get back to you shortly.</p>
        <p><strong>Subject:</strong> ${escapeHtml(data.subject)}</p>
  ...
  results.forEach((result) => {
    if (result.status === "rejected") {
      console.error("contact-request: failed to send email", result.reason);
    }
  });
